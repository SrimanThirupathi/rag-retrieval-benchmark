"""Arm — TurboVec, pooled setting. Same shared corpus, 4-bit compressed index."""
import numpy as np
import config
import llm
...
def _get_index():
    ...
    if _index is None:
        ...
        _index = TurboQuantIndex(dim=emb.shape[1],
...
def retrieve(item, k=None):
    ...
    _, idx = index.search(q, k=k)
    return [corpus[i] for i in np.asarray(idx).reshape(-1)[:k]]
